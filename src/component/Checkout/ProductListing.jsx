...
export default function ProductListing() {
  ...
  return (
    <div
      className={`w-full flex flex-col items-center   justify-center text-center gap-6    ${
        theme === "light" ? "bg-pink-200 text-black" : "bg-dark"
      }`}
    >
      <div
      ...
      >
        ...
        <div
          className={`w-full mt-5 p-5 rounded ${
        ...
        >
          ...
          {products.map((product, index) => (
            <div
              className="w-[90%] lg:w-[100%] mx-auto grid grid-cols-7 justify-center items-center  py-3"
              key={index}
            >
              <div className=" col-span-7 lg:col-span-2 flex flex-col lg:flex-row  lg:items-center lg:justify-center gap-3 lg:gap-12 pb-2 lg:pb-0 px-3">
                <button onClick={() => deleteTodo(index)}>
                  <IoIosCloseCircleOutline className="w-6 h-6 text-gray-400 hover:text-black  lg:border-none" />
                </button>
